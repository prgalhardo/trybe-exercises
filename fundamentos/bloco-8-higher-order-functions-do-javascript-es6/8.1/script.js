...
}; // Nessa função como pede o retorno do nome completo com o e-mail da pessoa (sendo ele o nome completo), foi usado o toLowercase, split e join no fullName.
...
const notes = (rightAnswers, studentAnswers) => {
  let count = 0;
  for (let i = 0; i < rightAnswers.length; i += 1) {
    const rightNotes = rightAnswers[i];
    if (rightNotes === studentAnswers[i]) {
      count += 1
...
const testResult = (rightAnswers, studentAnswers, callback) => {
  return callback(rightAnswers, studentAnswers);
}
console.log(testResult(RIGHT_ANSWERS, STUDENT_ANSWERS, notes));